    ...
    <aside class="sidebar w-[240px] min-h-screen fixed top-0 left-0 bottom-0 flex flex-col py-7 px-4 z-40 overflow-y-auto">
        ...
        <nav class="space-y-0.5 flex-1">
            <a href="{{ route('dashboard') }}"
               class="nav-item flex items-center gap-3 px-3 py-2.5 rounded-xl text-emerald-100 hover:text-white text-sm font-semibold">
            ...
            <a href="{{ route('shipping-address') }}"
               class="nav-item active flex items-center gap-3 px-3 py-2.5 rounded-xl text-white text-sm font-semibold">
    ...
    <main class="ml-[240px] flex-1 min-h-screen p-8">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden max-w-3xl anim anim-d2">
            ...
            <form method="POST" action="{{ route('shipping-address.update') }}" class="px-8 py-7 space-y-5">
                ...
                <div>
                    ...
                    @error('full_address')
                        <p class="text-red-500 text-xs mt-1.5 flex items-center gap-1">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                            {{ $message }}
                        </p>
                    @enderror
                </div>
                ...
                <div>
                    <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">
                        Catatan Pengiriman <span class="text-gray-400 font-normal">(Optional)</span>
                    </label>
                ...
                <div class="flex items-center justify-end gap-3">
                    <a href="{{ route('dashboard') }}"
                       class="px-6 py-2.5 border border-gray-200 rounded-xl text-sm font-semibold text-gray-500 hover:bg-gray-50 transition">
                        Batal
                    </a>
                    <button type="submit"
