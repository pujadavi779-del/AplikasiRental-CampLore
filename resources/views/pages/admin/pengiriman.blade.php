        ...
        <div class="bg-white rounded-[28px] border border-[#d7e6de] shadow-sm overflow-hidden">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-[#eef4f0]">
                        {{-- MENGGUNAKAN $data_pengiriman SESUAI CONTROLLER --}}
                        @forelse($data_pengiriman as $item)
                        <tr class="hover:bg-[#fcfdfb] transition-colors group">
                            <td class="px-8 py-5 flex items-center gap-4">
                                ...
                                <div>
                                    <div class="font-bold text-[#22543D] text-sm">{{ $item->user->name }}</div>
                                    <div class="text-[9px] text-[#7c8b84] mt-1 italic uppercase tracking-wider">
                                        {{ $item->user->no_hp ?? 'No HP Tidak Ada' }}
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-5">
                                <p class="text-[11px] leading-relaxed max-w-[200px] text-[#4a5038]">
                                    {{ $item->alamat }}
                                </p>
                            </td>
                            <td class="px-6 py-5 text-center">
                                <div class="font-bold text-[#22543D] text-sm">{{ $item->product->name }}</div>
                                <div class="text-[9px] text-[#7c8b84] mt-1 italic tracking-wider uppercase">
                            ...
                            <td class="px-6 py-5 text-center">
                                <form action="{{ route('admin.admin.pengiriman.tiba', $item->id) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                        ...
                        @empty
                        <tr>
                            <td colspan="5" class="p-16 text-center text-xs text-[#7c8b84] italic tracking-wide">
            ...
            <div class="p-5 bg-[#fcfdfb] border-t border-[#eef4f0] flex items-center justify-between">
                <p class="text-[10px] font-bold text-[#22543D] uppercase tracking-widest">
                    Menampilkan {{ $data_pengiriman->count() }} data pengiriman
                </p>
